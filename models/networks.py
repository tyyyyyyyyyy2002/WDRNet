...
class PatchSampleF(nn.Module): #原来版本
...
class ResnetGenerator(nn.Module): #原本班
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        # for i in range(n_blocks):  # replace with WMB blocks
        #     model += [WMB(dim=ngf * mult)]  # WMB 接受通道维度
...
class SpatialTransformer(nn.Module):
    def __init__(self, channels, num_heads=8, context_dim=None):
        ...
        self.context_dim = context_dim if context_dim is not None else channels
    ...
    def forward(self, x, context=None):
        B, C, H, W = x.shape
        q = self.to_q(x).reshape(B, self.num_heads, C // self.num_heads, H * W)  # [B, heads, head_dim, HW]

        if context is None:
            # 如果没有 context，用 x 自身做 self-attention
            k = q
            v = q
        else:
            # 如果 context 是 list，取第一个元素（或者可以根据实际情况处理）
            if isinstance(context, list):
                context = context[0]

            # 确保 context 是 Tensor 并在同一设备
            if not torch.is_tensor(context):
                raise TypeError(f"Expected context to be Tensor, got {type(context)}")
            context = context.to(x.device, dtype=x.dtype)

            # context: [B, N, context_dim]
            k = self.to_k(context).reshape(B, self.num_heads, C // self.num_heads, -1)
            v = self.to_v(context).reshape(B, self.num_heads, C // self.num_heads, -1)

        attn = torch.einsum('bhcd,bhce->bhde', q, k) / (C // self.num_heads) ** 0.5
...
class DWTForward(nn.Module):
    """可微二维 Haar 小波变换，输出通道数 = 输入通道数 * 4"""
    def __init__(self):
        super(DWTForward, self).__init__()
        h0 = torch.tensor([1/torch.sqrt(torch.tensor(2.0)), 1/torch.sqrt(torch.tensor(2.0))])
        h1 = torch.tensor([1/torch.sqrt(torch.tensor(2.0)), -1/torch.sqrt(torch.tensor(2.0))])
        ll = torch.ger(h0, h0)
        lh = torch.ger(h0, h1)
        hl = torch.ger(h1, h0)
        hh = torch.ger(h1, h1)
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        filters = self.weight.repeat(C, 1, 1, 1)  # [C*4,1,2,2]
        y = F.conv2d(x, filters, stride=2, padding=0, groups=C)
        return y  # [B, C*4, H/2, W/2]

class MultiLevelDWT(nn.Module):
    """多尺度小波金字塔模块：先一级 DWT，再对 LL 子带再做一级 DWT"""
    def __init__(self):
        super(MultiLevelDWT, self).__init__()
        self.dwt1 = DWTForward()
        self.dwt2 = DWTForward()
    def forward(self, x):
        y1 = self.dwt1(x)  # Level1
        B, C4, H2, W2 = y1.shape
        C = C4 // 4
        ll = y1[:, 0:C, :, :]
        y2 = self.dwt2(ll)  # Level2
        return y1, y2
...
class BandAttention(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.size()
        # 确保输入通道与初始化一致
        assert C == self.in_channels, f"BandAttention in_channels mismatch: expected {self.in_channels}, got {C}"
        y = self.avg_pool(x).view(B, C)
        y = self.fc(y).view(B, C, 1, 1)
        return x * y.expand_as(x)

class NLayerDiscriminator(nn.Module):
    """多尺度小波金字塔 PatchGAN 判别器"""
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        self.spatial_branch = nn.Sequential(*sequence_spatial)

        # 频域分支 Scale1
        in_c1 = input_nc * 4
        self.wave1_branch = nn.Sequential(
            nn.Conv2d(in_c1, ndf, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            nn.LeakyReLU(0.2, True),
            BandAttention(ndf)  # 注意这里改成 ndf
        )
        nf_mult = 1
        for n in range(1, n_layers):
            nf_mult_prev = nf_mult
            nf_mult = min(2**n, 8)
            if no_antialias:
                self.wave1_branch.add_module(f"w1_conv_{n}", nn.Conv2d(ndf*nf_mult_prev, ndf*nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias))
                self.wave1_branch.add_module(f"w1_norm_{n}", norm_layer(ndf*nf_mult))
                self.wave1_branch.add_module(f"w1_lrelu_{n}", nn.LeakyReLU(0.2, True))
            else:
                self.wave1_branch.add_module(f"w1_conv_{n}", nn.Conv2d(ndf*nf_mult_prev, ndf*nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias))
                self.wave1_branch.add_module(f"w1_norm_{n}", norm_layer(ndf*nf_mult))
                self.wave1_branch.add_module(f"w1_lrelu_{n}", nn.LeakyReLU(0.2, True))
                self.wave1_branch.add_module(f"w1_pool_{n}", nn.AvgPool2d(2))
        self.wave1_branch.add_module("w1_final_conv", nn.Conv2d(ndf*nf_mult, ndf*nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias))
        self.wave1_branch.add_module("w1_final_norm", norm_layer(ndf*nf_mult))
        self.wave1_branch.add_module("w1_final_lrelu", nn.LeakyReLU(0.2, True))

        # 频域分支 Scale2
        in_c2 = input_nc * 4
        self.wave2_branch = nn.Sequential(
            nn.Conv2d(in_c2, ndf, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            nn.LeakyReLU(0.2, True),
            BandAttention(ndf)  # 注意这里改成 ndf
        )
        nf_mult = 1
        for n in range(1, n_layers):
            ...
            nf_mult = min(2**n, 8)
            if no_antialias:
                self.wave2_branch.add_module(f"w2_conv_{n}", nn.Conv2d(ndf*nf_mult_prev, ndf*nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias))
                self.wave2_branch.add_module(f"w2_norm_{n}", norm_layer(ndf*nf_mult))
                self.wave2_branch.add_module(f"w2_lrelu_{n}", nn.LeakyReLU(0.2, True))
            else:
                self.wave2_branch.add_module(f"w2_conv_{n}", nn.Conv2d(ndf*nf_mult_prev, ndf*nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias))
                self.wave2_branch.add_module(f"w2_norm_{n}", norm_layer(ndf*nf_mult))
                self.wave2_branch.add_module(f"w2_lrelu_{n}", nn.LeakyReLU(0.2, True))
                self.wave2_branch.add_module(f"w2_pool_{n}", nn.AvgPool2d(2))
        self.wave2_branch.add_module("w2_final_conv", nn.Conv2d(ndf*nf_mult, ndf*nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias))
        self.wave2_branch.add_module("w2_final_norm", norm_layer(ndf*nf_mult))
        self.wave2_branch.add_module("w2_final_lrelu", nn.LeakyReLU(0.2, True))

        # 最终融合
        fused_channels = (ndf * nf_mult) * 3
        self.final_conv = nn.Conv2d(fused_channels, 1, kernel_size=kw, stride=1, padding=padw)

    def forward(self, input):
        feat_spatial = self.spatial_branch(input)
        y1, y2 = self.multi_dwt(input)
        feat_wave1 = self.wave1_branch(y1)
        feat_wave2 = self.wave2_branch(y2)

        target_size = feat_spatial.shape[2:]
        if feat_wave1.shape[2:] != target_size:
            feat_wave1 = F.interpolate(feat_wave1, size=target_size, mode='bilinear', align_corners=False)
        if feat_wave2.shape[2:] != target_size:
            feat_wave2 = F.interpolate(feat_wave2, size=target_size, mode='bilinear', align_corners=False)

        feat = torch.cat([feat_spatial, feat_wave1, feat_wave2], dim=1)
