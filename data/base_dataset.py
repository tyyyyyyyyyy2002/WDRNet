...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    # if opt.preprocess == 'resize':  # ① 仅匹配纯粹的 resize
    #     # transforms.Resize((height, width))，注意先高后宽
    #     transform_list.append(transforms.Resize((540, 960), method))
    #
    # elif 'resize' in opt.preprocess:  # ② 其余包含 resize 的模式，如 resize_and_crop
    #     osize = [opt.load_size, opt.load_size]
    #     if "gta2cityscapes" in opt.dataroot:  # 这段原来就有，保留
